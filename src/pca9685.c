...
bool pca9685_set_all_channels(pca9685_handle_t *handle, unsigned on_time, unsigned off_time)
{
	assert(on_time >= 0);
	assert(on_time <= 4096);
	...
	uint8_t data[1] = { on_time & 0xFF};
	success &= pca9685_write_data(handle, PCA9685_REGISTER_ALL_LED_ON_L, data, 1);

	data[0] = on_time >> 8u;
	success &= pca9685_write_data(handle, PCA9685_REGISTER_ALL_LED_ON_H, data, 1);

	data[0] = off_time & 0xFF;
	success &= pca9685_write_data(handle, PCA9685_REGISTER_ALL_LED_OFF_L, data, 1);

	data[0] = off_time >> 8u;
	success &= pca9685_write_data(handle, PCA9685_REGISTER_ALL_LED_OFF_H, data, 1);
